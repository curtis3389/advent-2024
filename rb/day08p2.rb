...
class Map
  ...
  private def find_antinodes(location)
    ...
    (0..max_dist).flat_map do |distance|
      ...
      antinode_frequencies = current.select {|pair| possibles.any? {|other| other[1] == pair[1] && ((slope(location, pair[0]) - slope(location, other[0])).abs < 1e-9 || slope(location, pair[0]).nan?)}}.map {|pair| pair[1]}.uniq
  ...
  def test
    p frequencies_at([1,8], 0)
    p frequencies_at([0,0], 0)
