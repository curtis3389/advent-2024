...
paths = map.all_paths
pairs = paths.map {|path| [path.first, path.last]}.uniq
trailheads = pairs.map {|pair| pair[0]}.uniq
puts trailheads.map {|trailhead| pairs.select {|pair| pair[0] == trailhead}.count}.sum
