...
class Map
  ...
  def find_antinodes(location)
    max_dist = [@height, @width].max
    (1..max_dist).flat_map do |distance|
      closer = frequencies_at(location, distance)
      farther = frequencies_at(location, distance * 2)
      antinode_frequencies =
        closer
        .select do |pair|
          farther.any? do |other|
            other[1] == pair[1] && (slope(location, pair[0]) - slope(location, other[0])).abs < 1e-9
          end
        end
        .map { |pair| pair[1] }
        .uniq
